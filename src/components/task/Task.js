					...
					>
						<div className='task-header'>
							<div
								...
								style={{  border: `2px solid ${importanceColors[importance]}` }}></div>
							<h3 className="task-name">{task.length > 80 ? task.slice(0, 80) + '...' : task }</h3>
							<div
						...
						<p className='task-description'>{ description.length > 80 ? description.slice(0, 80) + '...' : description }</p>
					</div>
				))}
			</div>
		);
